...
## Four services, one app

OnFlip drives one service at a time, switched from the account menu — the app
...
**Does this break the service's terms?** OnFlip drives a real browser session as a logged-in user and bypasses no checks — Cloudflare challenges are completed by you, in a real window. You are responsible for using your accounts within each service's terms.
