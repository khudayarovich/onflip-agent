...
[Attached to this message: ${attachments
    ...
    try {
      ...
      if (inScratch(this.cwd)) {
        try {
          const files = scratchArtifacts(this.cwd, turnBeganAt);
          if (files.length > 0) {
...
/** Files in a scratch workspace the agent touched during this turn. */
export function scratchArtifacts(dir: string, since: number, cap = 20): { name: string; path: string; size: number }[] {
  ...
  const walk = (d: string, depth: number): void => {
    ...
    for (const entry of entries) {
      ...
      try {
        ...
        // A second of slack: tool clocks and fs timestamps round differently.
        if (stat.mtimeMs >= since - 1_000) {
          out.push({ name: path.relative(dir, full), path: full, size: stat.size });
        }
      } catch {
        /* deleted mid-scan */
      }
